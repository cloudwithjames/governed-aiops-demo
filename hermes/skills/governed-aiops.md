...
- `check_portal` — Check Customer Portal HTTP status (read-only)
- `check_backend` — Check backend health endpoint (read-only)
- `get_gateway_logs` — Get recent gateway error logs (read-only)
- `get_gateway_config` — Get current gateway upstream configuration (read-only)
- `get_recent_changes` — Get recent configuration change history (read-only)
- `apply_gateway_fix` — Apply gateway upstream fix (requires approval token)
- `verify_service` — Verify service health after remediation (read-only)

## Workflow

### Phase A — Diagnose and Plan

1. Call read-only MCP tools to gather evidence
2. Determine root cause citing evidence IDs
3. Build remediation plan with action/impact/risk/blast radius/rollback/post-checks
4. Stop before any write action

### Phase B — Execute and Verify

1. Execute only the approved remediation via `apply_gateway_fix`
2. Call `verify_service`
3. Do not declare success unless verification passes
